...
void LA4Compressor::process(juce::AudioBuffer<float>& buffer)
{
    ...
    const float fastAtk = timeToCoeff(3.0f,    sampleRate);
    const float fastRel = timeToCoeff(80.0f,   sampleRate);
    const float slowAtk = timeToCoeff(40.0f,   sampleRate);
    const float slowRel = timeToCoeff(1800.0f, sampleRate);
    const float gAtk    = timeToCoeff(5.0f,    sampleRate);
    const float gRel    = timeToCoeff(380.0f,  sampleRate);
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        // Dual opto time-constant simulation
        ...
        // Weighted photocell output: predominantly the slow component
        ...
        // Gentle 2nd-harmonic transformer warmth
